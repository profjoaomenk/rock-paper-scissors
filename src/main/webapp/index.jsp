...
<script>
    ...
    function GameService() {
        ...
        this.playGame = function (input) {
            ...
            const jogadasKeys = Object.keys(jogadas)
            const jogadaComputador = Math.floor(Math.random() * jogadasKeys.length)

            if ((modulo(jogadas[input] - jogadas[jogadasKeys[jogadaComputador]], jogadasKeys.length) % 2) === 1) {
                result = "win"
                theScore.increaseWins();
            } else if (jogadas[input] - jogadas[jogadasKeys[jogadaComputador]] === 0) {
                result = "tie"
            ...
            let gameSummary = new GameSummary(input, jogadasKeys[jogadaComputador], result);
